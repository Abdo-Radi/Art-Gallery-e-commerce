...
const EditArtwork = ({ artwork, onCancel }) => {
    ...
    const [formData, setFormData] = useState({ title, artist, category, price, description })
    ...
    const onSubmit = data => {
        ...
        dataToUpload.append("title", data.title)
        dataToUpload.append("artist", data.artist)
        dataToUpload.append("category", data.category)
        dataToUpload.append("price", data.price)
        dataToUpload.append("description", data.description)

        if (selectedImage)
            dataToUpload.append("image", selectedImage)

        dispatch(editArtwork({ id: artwork._id, body: dataToUpload }))
        onCancel()
    }
                    ...
                    <div className="mb-4.5 relative z-20 bg-transparent dark:bg-form-input">
                        ...
                        >
                            ...
                            {artists && artists.map((artist, key) => (
                                <option key={key} value={artist._id} className="text-body dark:text-bodydark">
                    ...
                    <div className="mb-4.5 relative z-20 bg-transparent dark:bg-form-input">
                        ...
                        >
                            ...
                            {categories && categories.map((category, key) => (
                                <option key={key} value={category._id} className="text-body dark:text-bodydark">
                                    {category.name}
                                </option>
                            ))}
                        </select>
